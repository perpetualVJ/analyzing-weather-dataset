...
def categorical(df):
    ...
    return list(categorical_var.columns)
...
def numerical(df):
    ...
    return list(numerical_var.columns)
...
def instances_based_condition(df,col1,val1,col2,val2):
    ...
    """
    df1 = df[(df[col1] > val1) & (df[col2] == val2)]
    return df1
...
def agg_values_ina_month(df,date_col,agg_col, agg):
    ...
    """
    #df1 = df[date_col].apply(lambda x : x.split(sep = '-'))
...
dic = ['mean','max', 'min', 'sum','count']
aggregated_value = agg_values_ina_month(weather, 'Date/Time', 'Temp (C)', dic)
